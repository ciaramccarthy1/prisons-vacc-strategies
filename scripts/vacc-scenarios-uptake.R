...
new1 <- n_vacc_daily/(sum(params$pop[[1]]$size)*staff_to*uptake.staff)
new2 <- n_vacc_daily/(sum(params$pop[[2]]$size)*staff_to*uptake.staff)
new3 <- n_vacc_daily/(sum(params$pop[[3]]$size)*b_rate*uptake.pris)

new1.7 <- sum(vacc_vals7.pop1)/(sum(params$pop[[1]]$size)*staff_to*uptake.staff)
new2.7 <- sum(vacc_vals7.pop2)/(sum(params$pop[[2]]$size)*staff_to*uptake.staff)
new3.7 <- sum(vacc_vals7.pop3)/(sum(params$pop[[3]]$size)*b_rate*uptake.pris)
...
# scenarios
 ...
 #  results_run3 = run3$dynamics[compartment == "cases_i", .(total = sum(value)), by = .(run, population, t)] %>% mutate(scenario_run="run3")
  ...
  total.popn <-  sum(params$pop[[1]]$size[11:16]+params$pop[[2]]$size[11:16])+sum(params$pop[[3]]$size[11:16])
  ## Number of doses needed for this strategy:
