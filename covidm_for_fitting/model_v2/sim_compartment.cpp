...
void Population::Tick(Parameters& P, Randomizer& Rand, double t, vector<double>& infec, Reporter& rep)
{
    ...
    for (unsigned int a = 0; a < lambda.size(); ++a)
    {
        ...
        // S -> V, V -> S, R -> S
        double nS_V   = min(S[a],        num(P.pop[p].v[a]   * P.pop[p].ev[a] * P.time_step));
        double nS_V2  = min(S[a] - nS_V, num(P.pop[p].v2[a]  * P.pop[p].ev2[a] * P.time_step));
        double nV_V2  = min(V[a],        num(P.pop[p].v12[a] * P.pop[p].ev2[a] * P.time_step));
        double nV_S   = V[a]*(P.pop[p].wv[a]  * P.time_step);
        double nV2_S  = V2[a]*(P.pop[p].wv2[a] * P.time_step);
        double nR_S   = R[a]*(P.pop[p].wn[a]  * P.time_step);
        ...
        for (auto& process : P.processes)
        {
            ...
            switch (process.source_id)
            {
                ...
                case srcV:
                    n_entering = nS_V; break;
                case srcV2:
                    n_entering = nV_V2 + nS_V2; break;
                default:
    ...
    for (unsigned int a = N.size() - 1; ; --a)
    {
        ...
        double B = Ntot * P.pop[p].B[a] * P.time_step;
        double BS  = B;
        ...
        double death_prob = P.pop[p].D[a] * P.time_step;
        double DS   = S[a] * death_prob;
        double DV   = V[a] * death_prob;
        double DV2  = V2[a] * death_prob;
        double DR   = R[a] * death_prob;
